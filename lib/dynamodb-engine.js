...
API.createRelation = function createRelation(subject, object) {
	...
	if (!U.isFullString(subject.id)) {
		throw new Error('#createRelation(subject, object) subject.id is a ' +
			'required String in DynamoDB Engine');
	}
	if (!U.isFullString(subject.type)) {
		throw new Error('#createRelation(subject, object) subject.type is a ' +
			'required String in DynamoDB Engine');
	}
	if (!U.isFullString(object.id)) {
		throw new Error('#createRelation(subject, object) object.id is a ' +
			'required String in DynamoDB Engine');
	}
	if (!U.isFullString(object.type)) {
		throw new Error('#createRelation(subject, object) object.type is a ' +
			'required String in DynamoDB Engine');
	}
	...
	debug('create relation %s %s %s %s', subject.type, subject.id, object.type, object.id);
	return this.dynamodb
		.putItem(params)
		.then(function () {
			debug('create relation %s %s %s %s', subject.type, subject.id, object.type, object.id);
...
API.getReverseRelations = function getRelations(objectId, predicate) {
	if (!U.isFullString(objectId)) {
		throw new Error('#getReverseRelations(objectId, predicate) objectId is a ' +
			'required String in DynamoDB Engine');
	}
	if (!U.isFullString(predicate)) {
		throw new Error('#getReverseRelations(objectId, predicate) predicate is a ' +
			'required String in DynamoDB Engine');
	}
	...
	debug('get reverse relations %s %s', objectId, predicate);
	return query.rangeEqual(predicate).fetchAll()
		.then(function (items) {
			debug('got reverse relations %s %s', objectId, predicate);
			return items
				.map(function (item) {
					return {
						id: item.subjectId,
