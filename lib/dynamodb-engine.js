...
utils.extend(DynamoDBEngine.prototype, {
	...
	//   idAttribute: 'STRING_VALUE', /* "id" is the default */
	// }
	...
	createGenericTable: function (params) {
		...
		return new Promise(function (resolve, reject) {
			...
			self.emit('log', {
				...
				message: 'attempting to create table "' + newParams.TableName + '"'
			});
	...
	deleteTable: function (params) {
		...
		var newParams = {
			TableName: params.tableName || params.TableName || this.tableName
		};
		...
		return new Promise(function (resolve, reject) {
			...
			self.emit('log', {
				level: LOGLEVELS_INFO,
				message: 'attempting to delete table "' + newParams.TableName + '"'
			});
			self.dynamodb.deleteTable(newParams, function (err) {
				if (err) {
					if (err.code === 'ResourceNotFoundException') {
						self.emit('log', {
							level: LOGLEVELS_WARN,
							message: 'attempting to delete table; "' +
								newParams.TableName + '" does not exist'
						});
					} else {
						reject(err);
					}
	...
	listTables: function () {
		...
		return new Promise(function (resolve, reject) {
			self.emit('log', {
				level: LOGLEVELS_INFO,
				message: 'attempting to list all tables'
			});
...
DynamoDBEngine.serializeRecord = function (record, options) {
	...
	return Object.keys(record.data).reduce(function (rec, key) {
		var val;
		if (key !== idAttribute) {
			val = DynamoDBEngine.typeCast(record.data[key]);
			if (val) {
				rec[key] = val;
			}
		}
