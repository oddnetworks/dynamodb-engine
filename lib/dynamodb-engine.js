...
var BatchGet = require('./batch_get');
		...
		return dynamodb.describeTable({TableName: tableName})
			.then(function (res) {
				...
				if (spec.indexes && spec.indexes.length) {
					...
					if (updateDelta) {
						return dynamodb.updateTable(updateDelta);
					}
				}
				return res.Table;
			})
...
API.batchGetRecords = function batchGetRecord(query) {
	if (!query || !U.isObject(query)) {
		throw new Error('#batchGetRecord(query) query is a ' +
			'required Object in DynamoDB Engine');
	}

	var self = this;

	var spec = Object.keys(query).reduce(function (spec, type) {
		var keys = query[type];
		if (!Array.isArray(keys)) {
			throw new Error('#batchGetRecord(query) query[Type] must be an ' +
				'Array in DynamoDB Engine');
		}

		spec[type] = keys.map(function (key) {
			if (!U.isString(key) && !U.isNumber(key)) {
				throw new Error('#batchGetRecord(query) query[Type] keys must be an ' +
					'Array of Strings or Numbers in DynamoDB Engine');
			}
			return {id: key};
		});
	...
	var batchGet = BatchGet.create({
		dynamodb: this.dynamodb
	});

	debug('batch get from %s', Object.keys(query).join(', '));
	return batchGet.fetchAll(spec)
		.then(function (res) {
			debug('batch get success');
			return res;
		})
		.catch(this.NonExistentTableError, function () {
			debug('batch get from %s -- missing table', Object.keys(query).join(', '));
...
API.getRelations = function getRelations(subjectId, predicate) {
	...
	return query.rangeEqual(predicate).fetchAll()
		.then(function (items) {
		...
		.catch(this.OperationalError, function (err) {
			...
			if (/^Table ([\w]+) does not exist/.test(err.message)) {
				debug('get relations %s %s -- missing table', subjectId, predicate);
				msg = 'Entity table for relations does not exist.';
				msg += ' A migration is probably required.';
				return Promise.reject(new self.OperationalError(msg));
			}
			if (/^Index ([\w]+) does not exist/.test(err.message)) {
				debug('get relations %s %s -- missing index', subjectId, predicate);
...
API.removeRelation = function removeRelation(subjectId, objectId) {
	if (!subjectId || !U.isString(subjectId)) {
		throw new Error('#removeRelation(subjectId, object) subjectId is a ' +
			'required String in DynamoDB Engine');
	}
	if (!objectId || !U.isString(objectId)) {
		throw new Error('#removeRelation(subjectId, object) object.id is a ' +
			'required String in DynamoDB Engine');
	}

	var params = {
		Key: DynamoDB.serializeKey({subject: subjectId, object: objectId}),
		TableName: this.dynamodb.relationTable(),
		ReturnValues: 'NONE'
		// Potentially useful parameters:
		// ReturnConsumedCapacity
	...
	debug('remove relation %s %s', subjectId, objectId);
	return this.dynamodb
		.deleteItem(params)
		.then(function () {
		...
		.catch(this.NonExistentTableError, function () {
			debug('remove relation %s %s -- missing table', subjectId, objectId);
			var msg = 'Entity table for relations does not exist.';
			msg += ' A migration is probably required.';
			return Promise.reject(new self.OperationalError(msg));
		});
...
// args.type
// args.index
API.query = function query(args) {
	if (!args.type || !U.isString(args.type)) {
		throw new Error('#query(args) args.type is a ' +
			'required String in DynamoDB Engine');
	}
	if (!args.index || !U.isString(args.index)) {
		throw new Error('#query(args) args.index is a ' +
			'required String in DynamoDB Engine');
	}

	var msg;
	var tableName = this.dynamodb.table(args.type);
	var table = this.schema[tableName];
	if (!table) {
		msg = 'No Table is defined in the schema for ' + args.type;
		throw new this.OperationalError(msg);
	}
	var indexes = table.indexes;
	if (!indexes || !indexes.length) {
		msg = 'No indexes are defined in the schema for ' + args.type;
		throw new this.OperationalError(msg);
	}
	var indexName = this.dynamodb.index(args.type, args.index);
	var index = U.find(indexes, {indexName: indexName});
	if (!index) {
		msg = 'Index ' + args.index + ' not defined in the schema for ' + args.type;
		throw new this.OperationalError(msg);
	}
	...
	return Query.create({
		...
		tableName: tableName,
		indexName: indexName,
		hashkey: index.keys.hash,
		rangekey: index.keys.range
	});
};
