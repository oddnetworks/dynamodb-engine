...
API.batchGetRecords = function batchGetRecord(query) {
	...
	var spec = Object.keys(query).reduce(function (spec, type) {
		...
		spec[table] = keys.map(function (key) {
	...
	return batchGet.fetchAll(spec)
		.then(function (res) {
			...
			var results = Object.keys(res).reduce(function (items, table) {
				var type = tables[table];
				items[type] = res[table].map(DynamoDB.deserializeItem);
				count += items[type].length;
				return items;
			}, Object.create(null));
			debug('batch get success %d items from %s', count, Object.keys(query).join(', '));
			return results;
		})
		.catch(this.NonExistentTableError, function () {
...
API.removeRelation = function (subjectId, objectId) {
	if (!subjectId || !U.isString(subjectId)) {
