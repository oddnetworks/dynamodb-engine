...
var RECORD_EXISTS_ERROR = constants.get('RECORD_EXISTS_ERROR');
...
function RecordExistsError(id) {
	Error.call(this);
	Error.captureStackTrace(this, this.constructor);
	this.name = this.constructor.name;
	this.code = RECORD_EXISTS_ERROR;
	this.message = 'Record id ' + id + ' already exists';
}
util.inherits(RecordExistsError, OperationalError);
errors.RecordExistsError = RecordExistsError;
