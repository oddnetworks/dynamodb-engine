...
U.extend(TableSchema.prototype, {
	...
	updateDelta: function (current) {
		...
		var indexes = this.GlobalSecondaryIndexes.reduce(function (indexes, spec) {
			...
			} else if (!U.isEqual(currentIndex.KeySchema, spec.KeySchema)) {
		...
		if (indexes.length > 0) {
			...
			if (current.StreamSpecification) {
				update.StreamSpecification = current.StreamSpecification;
			}
			if (indexes && indexes.length > 0) {
				update.GlobalSecondaryIndexUpdates = indexes;
			}
		}
...
TableSchema.create = function (spec) {
	...
	var gsi;
	if (spec.GlobalSecondaryIndexes && spec.GlobalSecondaryIndexes.length > 1) {
		gsi = spec.GlobalSecondaryIndexes.map(GlobalSecondaryIndex.create);
	} else {
		gsi = [];
	}
