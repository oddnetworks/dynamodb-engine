...
exports.keySchema = function keySchema(keys, attributes) {
...
exports.valueDefinition = function valueDefinition(obj) {
	switch (typeof obj) {
		case 'string':
			return {S: obj};
		case 'number':
		...
		case 'boolean':
			return {B: obj};
		default:
			throw new Error('DynamoDB Engine epxects values to be ' +
...
// record.data
// options.idAttribute
exports.serializeRecord = function serializeRecord(attrs) {
	return Object.keys(attrs).reduce(function (rec, key) {
		var val = exports.typeCast(attrs[key]);
		if (val) {
			rec[key] = val;
		}
		return rec;
...
exports.isConditionalFailedException = function (err) {
