...
U.extend(DynamoDB.prototype, {
	...
	table: function (type) {
		return this.tablePrefix + '_' + type + '_entities';
	},

	relationTable: function () {
		return this.tablePrefix + '_relations';
	},
...
DynamoDB.keySchema = function keySchema(keys, attrs) {
	...
	if (keys.hash) {
		...
		if (!U.find(attrs, 'AttributeName', keys.hash)) {
	...
	if (keys.range) {
		...
		if (!U.find(attrs, 'AttributeName', keys.range)) {
